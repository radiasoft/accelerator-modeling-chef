...
template<typename stored_type, typename node_compare_type, typename node_order_compare_type>
typename unique_tree<stored_type, node_compare_type, node_order_compare_type>::child_iterator 
unique_tree<stored_type, node_compare_type, node_order_compare_type>::insert(const stored_type& element) 
{ 
	const tree_type* const pRoot = get_root();
	if ( allow_orphans() && pRoot->pOrphans ) { // orphans present?
		// yes.  check orphans for child
		typename associative_tree_type::iterator oit = pRoot->pOrphans->find_deep(element);
		if ( oit != pRoot->pOrphans->end() ) { 
			// child is an orphan.  update orphan with new data
			oit.node()->set(stored_type(element));
			tree_type orphan;
			orphan.set(*oit.node());
	...
	if ( !check_for_duplicate(element, this)) { // check for duplication
		const typename associative_tree_type::iterator it = associative_tree_type::insert(element, this);
...
template<typename stored_type, typename node_compare_type, typename node_order_compare_type>
typename unique_tree<stored_type, node_compare_type, node_order_compare_type>::child_iterator 
unique_tree<stored_type, node_compare_type, node_order_compare_type>::insert( const stored_type& parent_obj, const stored_type& element)
{
	if ( !(parent_obj < *this->get()) && !(*this->get() < parent_obj) ) { // is this node the parent?	
		return insert(element);  // yes.  insert the node here.
	}
	...
	if ( it_parent != associative_tree_type::end() ) {
		...
		if ( allow_orphans() && pRoot->pOrphans ) {
			...
			if ( oit != pRoot->pOrphans->end() ) {
				...
				it = it_parent.node()->insert(element); // child not an orphan. inset child node in parent 
		} else {
			it = it_parent.node()->insert(element); // no orphans.  insert child node in parent
		}
		...
		typename associative_tree_type::iterator child_oit = pRoot->pOrphans->find_deep(element);
		if ( child_oit != pRoot->pOrphans->end() ) {
			// child is an orphan.  update orphan with new data
			child_oit.node()->set(stored_type(element));
			tree_type orphan;
			...
			it = oit.node()->insert(element); // child not an orphan.  insert child in parent orphan
			oit.node()->ordered_children.clear();
		}
...
template<typename stored_type, typename node_compare_type, typename node_order_compare_type>
typename unique_tree<stored_type, node_compare_type, node_order_compare_type>::child_iterator 
unique_tree<stored_type, node_compare_type, node_order_compare_type>::find_deep(const stored_type& element) 
{
	tree_type tree_node(element);  // create seach node
	...
	typename associative_tree_type::iterator it = associative_tree_type::find(element);
	...
	for ( ; it != it_end; ++it ) {  // iterate over children and call this fcn recursively
		const typename associative_tree_type::iterator grandchild_it = it.node()->find_deep(element);
...
template<typename stored_type, typename node_compare_type, typename node_order_compare_type>
typename unique_tree<stored_type, node_compare_type, node_order_compare_type>::const_child_iterator 
unique_tree<stored_type, node_compare_type, node_order_compare_type>::find_deep(const stored_type& element) const
{
	...
	// node is some type of descendant.  check if it's an immediate child
	typename associative_tree_type::const_iterator it = associative_tree_type::find(element);
	if ( it != it_end )
		return it;
	...
	it = associative_tree_type::begin();
	for ( ; it != it_end; ++it ) { // iterate over children and call this fcn recursively
		typename associative_tree_type::const_iterator grandchild_it = it.node()->find_deep(element);
...
unique_tree<stored_type, node_compare_type, node_order_compare_type>::find_ordered(const stored_type& element) 
{
	tree_type tree_node(element);  // search node
...
unique_tree<stored_type, node_compare_type, node_order_compare_type>::find_ordered(const stored_type& element) const
{
	tree_type tree_node(element);  // search node
	return associative_tree_type::const_ordered_iterator(ordered_children.find(&tree_node));
}

// erase(const stored_type&)
template<typename stored_type, typename node_compare_type, typename node_order_compare_type>
bool unique_tree<stored_type, node_compare_type, node_order_compare_type>::
erase(const stored_type& element)
{
	const typename associative_tree_type::iterator it = find_deep(element);  // see if node is a descendant
	if ( it != associative_tree_type::end() ) {
...
// erase(iterator)
template<typename stored_type, typename node_compare_type, typename node_order_compare_type>
void unique_tree<stored_type, node_compare_type, node_order_compare_type>::erase(typename associative_tree<stored_type, unique_tree<stored_type, node_compare_type, node_order_compare_type>,  std::set<unique_tree<stored_type, node_compare_type, node_order_compare_type>*, unique_tree_deref_less<stored_type, node_compare_type, node_order_compare_type> > >::iterator it) 
{
...
template<typename stored_type, typename node_compare_type, typename node_order_compare_type>
void unique_tree<stored_type, node_compare_type, node_order_compare_type>::erase(typename associative_tree<stored_type, unique_tree<stored_type, node_compare_type, node_order_compare_type>,  std::set<unique_tree<stored_type, node_compare_type, node_order_compare_type>*, unique_tree_deref_less<stored_type, node_compare_type, node_order_compare_type> > >::iterator it_beg, typename associative_tree<stored_type, unique_tree<stored_type, node_compare_type, node_order_compare_type>,  std::set<unique_tree<stored_type, node_compare_type, node_order_compare_type>*, unique_tree_deref_less<stored_type, node_compare_type, node_order_compare_type> > >::iterator it_end) 
{
	while (it_beg != it_end) {
		erase(it_beg++);
	}
...
// check_for_duplicate()
template<typename stored_type, typename node_compare_type, typename node_order_compare_type>
bool unique_tree<stored_type, node_compare_type, node_order_compare_type>::check_for_duplicate(const stored_type& element, const tree_type* pParent) const
{
	...
	if (!(element < *pParent->get()) && !(*pParent->get() < element))
		return true;

	typename associative_tree_type::const_iterator it = pParent->find_deep(element);  // check if node is descendant of root
...
// swap
template<typename stored_type, typename node_compare_type, typename node_order_compare_type>
void unique_tree<stored_type, node_compare_type, node_order_compare_type>::swap(tree_type& rhs)
{
	tree_type temp(*this);
