...
public:
	...
	template<typename iterator_type> multitree(iterator_type it_beg, iterator_type it_end, const stored_type& element = stored_type()) : associative_tree_type(element) { while (it_beg != it_end) insert(*it_beg++); }
