...
class tree : public associative_tree<stored_type, tree<stored_type, node_compare_type>,  std::set<tree<stored_type, node_compare_type>*, tree_deref_less<stored_type, node_compare_type> > >
{
public:
	// typedefs
	...
	explicit tree( const stored_type& element ) : associative_tree_type(element) {}
	template<typename iterator_type> tree(iterator_type it_beg, iterator_type it_end, const stored_type& element = stored_type()) : associative_tree_type(element) { while (it_beg != it_end) insert(*it_beg++); }
