...
// swap
template<typename stored_type>
void sequential_tree<stored_type>::swap(tree_type& rhs)
{
	tree_type temp(*this);
...
template< typename stored_type>
typename sequential_tree<stored_type>::iterator 
sequential_tree<stored_type>::insert( const stored_type& element)
{
...
template< typename stored_type>
typename sequential_tree<stored_type>::iterator 
sequential_tree<stored_type>::insert(const tree_type& tree_obj)
{
	// insert current node
...
template< typename stored_type>
void sequential_tree<stored_type>::push_back(const stored_type& element)
{
	// create a new tree_type object to hold the node object
	sequential_tree<stored_type>* pNew_node; 
	allocate_tree_type(pNew_node, tree_type(element));
...
template<typename stored_type>
typename sequential_tree<stored_type>::iterator 
sequential_tree<stored_type>::insert(const_iterator pos, const stored_type& element)
{
	// create a new tree_type object to hold the node object
	tree_type* pNew_node = 0; 
	basic_tree_type::allocate_tree_type(pNew_node, tree_type(element));
	pNew_node->set_parent(this);
...
template<typename stored_type>
void 
sequential_tree<stored_type>::insert(const_iterator pos, const size_type num, const stored_type& element)
{
...
template<typename stored_type>
typename sequential_tree<stored_type>::iterator 
sequential_tree<stored_type>::insert(const const_iterator pos, const tree_type& tree_obj)
{
	// insert current node
...
// set(const tree_type&)
template< typename stored_type >
void sequential_tree<stored_type>::set(const sequential_tree<stored_type>& tree_obj)
{
...
// clear()
template< typename stored_type >
void sequential_tree<stored_type>::clear()
{
	iterator it = begin();
...
template<typename stored_type>
typename sequential_tree<stored_type>::iterator 
sequential_tree<stored_type>::erase(iterator it)
{
...
template<typename stored_type>
typename sequential_tree<stored_type>::iterator 
sequential_tree<stored_type>::erase(iterator beg_it, iterator end_it) 
{
	int delete_count = 0;
...
// operator [](size_type)
template<typename stored_type>
sequential_tree<stored_type>& sequential_tree<stored_type>::operator [](size_type index) 
{ 
...
template<typename stored_type>
const sequential_tree<stored_type>& sequential_tree<stored_type>::operator [](size_type index) const 
{ 
	if (index >= basic_tree_type::size())
		throw std::out_of_range("sequential_tree index out of range");
...
// operator ==
template<typename stored_type>
bool sequential_tree<stored_type>::operator == (const tree_type& rhs) const
{
	// check this node
...
// sort_descendants()
template<typename stored_type>
void sequential_tree<stored_type>::sort_descendants() 
{
	sort();
...
// sort_descendants(predicate)
template<typename stored_type>
void sequential_tree<stored_type>::sort_descendants(const pPred& predicate)
{
	sort(predicate);
	typename tree_type::post_order_iterator it = post_order_begin();
	const typename tree_type::post_order_iterator it_end = post_order_end();
	for ( ; it != it_end; ++it )
	{
		it.node()->sort(predicate);
	}
}
