  ...
  switch ( bel->kind_ ) {
    ...
    case BEL_HKICKER: {
      ...
      if ( length != 0.0 || kck != 0.0 ) {
        lbel = ElmPtr( new hkick( strip_final_colon( bel->name_ ).c_str(), length,  BRHO_*kck ) );
      } else {
        lbel = ElmPtr( new hkick( strip_final_colon( bel->name_ ).c_str(),  BRHO_*kck ) );
      }
    ...
    case BEL_VKICKER: {
      ...
      if ( length != 0.0 || kck != 0.0 ) {
        lbel = ElmPtr( new vkick( strip_final_colon( bel->name_ ).c_str(), length,  BRHO_*kck ) );
      } else {
        lbel = ElmPtr( new vkick( strip_final_colon( bel->name_ ).c_str(),  BRHO_*kck ) );
      }
    ...
    case BEL_KICKER: {
      ...
      if ( length != 0.0 || hkck != 0.0 || vkck != 0.0 ) {
        lbel = ElmPtr( new kick( strip_final_colon( bel->name_ ).c_str(), length, BRHO_*hkck, BRHO_*vkck ) );
      } else {
        lbel = ElmPtr( new kick( strip_final_colon( bel->name_ ).c_str(),  BRHO_*hkck,  BRHO_*vkck ) );
      }
    ...
    case BEL_ELSEPARATOR: {
      ...
      lbel = ElmPtr( new vkick( strip_final_colon( bel->name_ ).c_str(), length,  BRHO_*angle ) );
