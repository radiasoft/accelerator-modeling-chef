...
ETFncPlt::ETFncPlt( BeamlineContext* bcp, const char* name )
...
ETFncPlt::ETFncPlt( /* const */ beamline* pBml, const char* name )
: _errorStreamPtr(&cerr), _outputStreamPtr(&cout),
...
void ETFncPlt::_finishConstructor( const char* name )
{
  if( 0 != name ) {
    int len;
  ...
  strcat( theTitle, "Edwards-Teng lattice functions: " );
...
void ETFncPlt::setCaption( const char* cpt )
{
  _plotter->setCaption( cpt );
}


void ETFncPlt::_selfDestruct()
{
  delete this;
}
