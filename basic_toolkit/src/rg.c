...
/* rg.f -- translated by f2c (version of 3 February 1990  3:36:42).
...
/* L120: */
    ...
    s = (d_1 = h[en + na * h_dim1], abs(d_1)) + (d_2 = h[na + enm2 * h_dim1], 
	    abs(d_2));
    x = s * .75;
    y = x;
    w = s * -.4375 * s;
...
/*                for m=en-2 step -1 until l do -- .......... */
	...
	tst1 = abs(p) * ((d_1 = h[m - 1 + (m - 1) * h_dim1], abs(d_1)) + abs(
		zz) + (d_2 = h[m + 1 + (m + 1) * h_dim1], abs(d_2)));
...
L720:
	...
	cdiv_(&c_b60, &d_1, &d_2, &q, &h[na + na * h_dim1], &h[na + en * 
		h_dim1]);
L730:
	h[en + na * h_dim1] = 0.;
	h[en + en * h_dim1] = 1.;
...
L770:
	    ...
	    d_1 = -ra;
	    d_2 = -sa;
	    cdiv_(&d_1, &d_2, &w, &q, &h[i + na * h_dim1], &h[i + en * h_dim1]
		    );
	    goto L790;
...
L785:
	    ...
	    cdiv_(&d_1, &d_2, &zz, &q, &h[i + 1 + na * h_dim1], &h[i + 1 + en 
		    * h_dim1]);

/*     .......... overflow control .......... */
L790:
/* Computing MAX */
	    d_3 = (d_1 = h[i + na * h_dim1], abs(d_1)), d_4 = (d_2 = h[i + en 
		    * h_dim1], abs(d_2));
	    t = max(d_4,d_3);
	    if (t == 0.) {
		goto L795;
